...
fn get_content_length(request: &http::Request<Vec<u8>>) -> Result<Option<usize>, Error> {
    ...
    if let Some(header_value) = request.headers().get("content-length") {
        // If it exists, parse it as an usize (or return InvalidContentLength if it can't be parsed as such)
        Ok(Some(
            header_value
                .to_str()
...
///
/// You won't need to touch this function.
pub fn extend_header_value(request: &mut http::Request<Vec<u8>>, header_name: &'static str, appended_value: &str) {
    let new_value = match request.headers().get(header_name) {
        Some(existing_value) => {
            [existing_value.as_bytes(), b", ", appended_value.as_bytes()].concat()
        }
        None => appended_value.as_bytes().to_owned(),
    };

    //
    request.headers_mut().insert(header_name, http::HeaderValue::from_bytes(&new_value).unwrap());
}
